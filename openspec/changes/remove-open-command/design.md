...
- Update all ten affected specs, README, and tests.
...
The skill instructs the agent to read `taskflow.yaml` and present a ready-to-paste command instead of exec-ing the tool. Rationale: an agent shell typically has no PTY for an interactive TUI, and the user may want to add model/permission flags before launching. The skill composes:
...
- absolute paths only, so the pasted command works from any directory

### 3. `create --dry-run` is the readiness gate

`preflightOpen`'s checks (target exists, belongs to source common dir, expected branch; dirty is acceptable) are a subset of what create's reuse classification already verifies. The skill therefore instructs: run `taskflow create <task-id> --dry-run`; only when every repository reports `reuse` (no `create`, no conflict) should the agent compose and present the tool command. Zero new Go code; the "structurally ready" requirement keeps a deterministic check.
...
- [Ten-spec blast radius] Deltas touch many requirement texts → most edits are sentence-level removals; `development-tool-sessions` and `taskflow-multirepo-skill` carry the substantive rewrites.
