...
The test suite SHALL verify through the Cobra command surface and the compiled binary that create copies exactly the `.taskflowcopy`-declared paths, fails loudly when the manifest is missing, excludes `.git` entries from copied directories, and warns on literal patterns that match no source path.
...
- **WHEN** a source repository has no `.taskflowcopy` and create dry-run or execute runs
